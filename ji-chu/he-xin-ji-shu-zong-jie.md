...
"==" 操作符比较的是引用的地址（基本类型：比较的是值是否相同； 引用类型：比较的是引用是否相同；），而 "equals()" 方法比较的是对象的内容。

## 解释hashCode()与 equals()

hashCode() 和 equals() 都是 Java 中的方法，它们都是用来比较两个对象是否相等的。

hashCode() 方法是用来计算对象的哈希码的，它是通过对象中各个属性的值计算出来的一个整数。哈希码是用来在散列存储器中定位对象的，如果两个对象的哈希码相同，则它们在散列存储器中的位置也相同，因此它们可以被看作是相等的。在 Java 中，使用 == 运算符来比较两个对象是否相等时，实际上就是比较它们的哈希码是否相同。

equals() 方法是用来比较两个对象的内容是否相等的。它是在 Java 语言中定义的一个方法，用来比较两个对象的属性是否相同。当两个对象的属性都相同时，equals() 方法返回 true ；当两个对象的属性不相同时，equals() 方法返回 false 。
